...
function timeSpanToString(startDate, endDate) {
  ...
  return `${('0'.concat(hh)).slice(-2)}:${('0'.concat(mm)).slice(-2)}:${('0'.concat(ss)).slice(-2)}.${('00'.concat(sss)).slice(-3)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hours = 30 * (date.getUTCHours() % 12) + (minutes * 5) / 60;

  const result = Math.abs(hours - minutes);

  return result > 180 ? (((360 - result) * Math.PI) / 180) : (result * Math.PI) / 180;
}
